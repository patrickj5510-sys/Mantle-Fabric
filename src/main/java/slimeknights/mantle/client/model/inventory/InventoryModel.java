...
/** This model contains a list of multiple items to display in a block entity renderer. */
@AllArgsConstructor
public class InventoryModel implements IUnbakedGeometry<InventoryModel> {
  ...
  public BakedModel bake(IGeometryBakingContext owner, ModelBaker baker,
    ...
    return new Baked(model.bake(owner, baker, spriteGetter, transform, overrides), items);
  }
  ...
  public static InventoryModel deserialize(JsonObject json, JsonDeserializationContext context) {
    ...
    return new InventoryModel(model, ModelItem.listFromJson(json, "items"));
  }
}
